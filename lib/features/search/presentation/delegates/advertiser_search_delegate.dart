<<<<<<< HEAD:lib/screens/search/advertiser_search_delegate.dart
// import 'package:escort/screens/advertisers_screens/advertiser_public_profile.dart';
// import 'package:escort/services/advertiser_service.dart';
// import 'package:flutter/material.dart';
=======
/// Feature: Search
/// Delegate: AdvertiserSearchDelegate
///
/// Searches advertisers by name or @username and navigates to profile.
import 'package:escort/features/advertisers/presentation/screens/advertiser_public.dart';
import 'package:escort/services/advertiser_service.dart';
import 'package:flutter/material.dart';
>>>>>>> origin/master:lib/features/search/presentation/delegates/advertiser_search_delegate.dart

// class AdvertiserSearchDelegate extends SearchDelegate {
//   @override
//   String get searchFieldLabel => 'Search advertisers by name or @username';

//   @override
//   List<Widget> buildActions(BuildContext context) {
//     return [
//       if (query.isNotEmpty)
//         IconButton(
//           icon: const Icon(Icons.clear),
//           onPressed: () {
//             query = '';
//             showSuggestions(context);
//           },
//         ),
//     ];
//   }

//   @override
//   Widget buildLeading(BuildContext context) {
//     return IconButton(
//       icon: const Icon(Icons.arrow_back),
//       onPressed: () => close(context, null),
//     );
//   }

//   Future<List<Map<String, dynamic>>> _search(String q) async {
//     return AdvertiserService.search(q);
//   }

<<<<<<< HEAD:lib/screens/search/advertiser_search_delegate.dart
//   @override
//   Widget buildResults(BuildContext context) {
//     return FutureBuilder<List<Map<String, dynamic>>>(
//       future: _search(query),
//       builder: (context, snapshot) {
//         if (snapshot.connectionState == ConnectionState.waiting) {
//           return const Center(child: CircularProgressIndicator());
//         }
//         final items = snapshot.data ?? [];
//         if (items.isEmpty) {
//           return const Center(child: Text('No advertisers found', style: TextStyle(color: Colors.white70)));
//         }
//         return ListView.builder(
//           itemCount: items.length,
//           itemBuilder: (context, i) {
//             final a = items[i];
//             final name = (a['name'] ?? a['username'] ?? 'Advertiser').toString();
//             final username = (a['username'] ?? '').toString();
//             final location = (a['location'] ?? '').toString();
//             final avatar = (a['profile_image_url'] ?? '').toString();
//             final id = int.tryParse(a['id']?.toString() ?? '') ?? 0;
//             final validAvatar = (avatar.isNotEmpty &&
//                     !avatar.contains('via.placeholder.com') &&
//                     !avatar.contains('placeholder.com') &&
//                     !avatar.contains('picsum.photos'))
//                 ? avatar
//                 : '';
//             return ListTile(
//               leading: CircleAvatar(
//                 backgroundImage:
//                     validAvatar.isNotEmpty ? NetworkImage(validAvatar) : null,
//                 backgroundColor: Colors.grey[800],
//                 child: validAvatar.isEmpty ? const Icon(Icons.person, color: Colors.white70) : null,
//               ),
//               title: Text(name, style: const TextStyle(color: Colors.white)),
//               subtitle: Text('${username.isNotEmpty ? '@$username • ' : ''}$location', style: TextStyle(color: Colors.grey[400])),
//               trailing: const Icon(Icons.chevron_right, color: Colors.white70),
//               onTap: () {
//                 if (id > 0) {
//                   Navigator.of(context).push(
//                     MaterialPageRoute(builder: (_) => AdvertiserPublicProfileScreen(advertiserId: id)),
//                   );
//                 }
//               },
//             );
//           },
//         );
//       },
//     );
//   }
=======
  @override
  Widget buildResults(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: _search(query),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
        final items = snapshot.data ?? [];
        if (items.isEmpty) {
          return const Center(
            child: Text(
              'No advertisers found',
              style: TextStyle(color: Colors.white70),
            ),
          );
        }
        return ListView.builder(
          itemCount: items.length,
          itemBuilder: (context, i) {
            final a = items[i];
            final name = (a['name'] ?? a['username'] ?? 'Advertiser')
                .toString();
            final username = (a['username'] ?? '').toString();
            final location = (a['location'] ?? '').toString();
            final avatar = (a['profile_image_url'] ?? '').toString();
            final id = int.tryParse(a['id']?.toString() ?? '') ?? 0;
            final validAvatar =
                (avatar.isNotEmpty &&
                    !avatar.contains('via.placeholder.com') &&
                    !avatar.contains('placeholder.com') &&
                    !avatar.contains('picsum.photos'))
                ? avatar
                : '';
            return ListTile(
              leading: CircleAvatar(
                backgroundImage: validAvatar.isNotEmpty
                    ? NetworkImage(validAvatar)
                    : null,
                backgroundColor: Colors.grey[800],
                child: validAvatar.isEmpty
                    ? const Icon(Icons.person, color: Colors.white70)
                    : null,
              ),
              title: Text(name, style: const TextStyle(color: Colors.white)),
              subtitle: Text(
                '${username.isNotEmpty ? '@$username • ' : ''}$location',
                style: TextStyle(color: Colors.grey[400]),
              ),
              trailing: const Icon(Icons.chevron_right, color: Colors.white70),
              onTap: () {
                if (id > 0) {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) =>
                          AdvertiserPublicProfileScreen(advertiserId: id),
                    ),
                  );
                }
              },
            );
          },
        );
      },
    );
  }
>>>>>>> origin/master:lib/features/search/presentation/delegates/advertiser_search_delegate.dart

//   @override
//   Widget buildSuggestions(BuildContext context) {
//     return const SizedBox.shrink();
//   }
// }
